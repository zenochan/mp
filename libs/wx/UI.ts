...
export class UI {
  ...
  static showToast(msgOrOptions: String | Number | ToastOptions) {
    const options: ToastOptions = typeof msgOrOptions !== 'object'
      ? { title: `${msgOrOptions}` }
  ...
  static alert(content: string, confirm: string = '确定', title: string = '提示'): Rx.Observable<any> {
    ...
    const sub = new Rx.BehaviorSubject('ignore').filter((res) => res == true);
  ...
  static navColor(frontColor: '#ffffff' | '#000000', bgColor: String) {
    wx.setNavigationBarColor({ frontColor, backgroundColor: bgColor });
  }
  ...
  static toastSuccess(msg: string, complete = (() => undefined)) {
