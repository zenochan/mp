...
export class Nav {
  ...
  static nav(options: string | {
    ...
    const to = Route.create(op.url);
    const from = Route.create(WX.page().route);
  ...
  static navBack(data?: any) {
    ...
    if (prePage && prePage.holder) {
      ...
      prePage.zzLife().filter((res) => res === 'onShow').take(1).subscribe(() => {
        const cb = (prePage.holder as any).onResult;
