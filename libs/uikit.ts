import Rx = require("./rxjs-wx/Rx");
import {Observable} from "./rxjs-wx/Rx";
...
export class UIKit
{
  ...
  /** 不能通过点击遮罩消失 */
  static showModal(options: ModalOptions): Observable<boolean>
  {
    if (typeof options.content == "object") options.content = JSON.stringify(options.content);

    if (!options.confirmColor) options.confirmColor = UIKit.colorPrimary;
  ...
  /** 不能通过点击遮罩消失 */
  static confirm(content: string, confirm: string, title: string = "提示"): Observable<boolean>
  {
    if (typeof content == "object") content = JSON.stringify(content);
